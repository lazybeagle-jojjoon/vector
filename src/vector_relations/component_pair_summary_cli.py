...
from .component_pair_summary import ComponentPairSummaryOutputPaths, write_component_pair_summary_from_prices


def run(argv: Sequence[str] | None = None) -> ComponentPairSummaryOutputPaths:
    args = _parse_args(argv)
    prices, source_metadata = _read_inputs(args)
    return write_component_pair_summary_from_prices(
        prices=prices,
        ...
        cross_edge_threshold=args.cross_edge_threshold,
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    print(f"Wrote component pair metadata: {outputs.metadata_path}")
    print(f"Wrote component pair summary: {outputs.pair_summary_path}")
    print(f"Wrote component pair HTML: {outputs.html_path}")
    return 0
...
def _read_inputs(args: argparse.Namespace):
    ...
    source_metadata = {**source_metadata, "input_mode": "data_root_price_component_pair_windows"}
...
def _parse_args(argv: Sequence[str] | None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build same-window descriptive component-to-component relationship summaries."
    )
    ...
    parser.add_argument("--node-metadata", help="Accepted for input compatibility; ignored.")
    ...
    parser.add_argument("--output-dir", default="outputs/relation_snapshot_component_pair_summary")
...
def _import_pandas():
    try:
        ...
        raise ValueError(
            "component pair summary CSV mode requires pandas. Run with: "
            "uv run --no-project --with pandas python -m vector_relations.component_pair_summary_cli ..."
        ) from exc
    return pd
