...
[codex][component pair readout decision - 2026-06-06]

- Claude full-result review integration: apply a stricter readout over component-pair rows to avoid top-row cherry-picking and normalized-ratio over-reading. Reject lead-lag, propagation, predictive score, and single-window top-row claims.
- Implementation scope: filter same-window component pairs by mean cross-correlation, cross-edge density, pair count, component density, non-extreme market regime, and warrant-like symbol exclusion; then group adjacent windows by top-symbol overlap as a descriptive persistence aid.
- Stop line: this is not stable component identity tracking. Empty strict output is a valid result and should be read as "no pair passed all persistence/quality guards", not as a failure to find a signal.
